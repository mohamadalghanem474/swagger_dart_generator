...
class ProviderGenerator {
  ...
  String _getProviderPath(String featureName) {
    return switch (architectureStyle) {
      ArchitectureStyle.featureFirst => '$outputDir/lib/features/$featureName/presentation/providers',
      ArchitectureStyle.layerFirst => '$outputDir/lib/presentation/providers/$featureName',
  ...
  Future<void> _generateFeatureProviders(EndpointCategory category) async {
    ...
    final library = Library((b) {
      ...
      b.body.add(Field((b) => b
        ..name = '${StringUtils.toLowerCamelCase(category.name)}RepositoryProvider'
        ..type = refer('Provider<$repoInterface>')
        ..modifier = FieldModifier.final$
        ..assignment = refer('Provider').call([
          Method((b) => b
            ..requiredParameters.add(Parameter((b) => b..name = 'ref'))
            ..body = refer('${StringUtils.toPascalCase(packageName)}.init').call([refer('Dio()')]).property(StringUtils.toLowerCamelCase(category.name)).code
          ).closure
        ]).code
      ));
      ...
      for (final endpoint in category.endpoints) {
        ...
        final notifierName = '${StringUtils.toPascalCase(endpoint.methodName)}Notifier';
        final providerName = '${endpoint.methodName}Provider';
        
        b.body.add(Field((b) => b
          ..name = providerName
          ..modifier = FieldModifier.final$
          ..assignment = refer('AsyncNotifierProvider<$notifierName, dynamic>').call([
      ...
      b
        ...
        ..methods.add(Method((b) {
          ...
          if (endpoint.hasRequestBody || endpoint.queryParams.isNotEmpty || endpoint.pathParams.isNotEmpty) {
            b.requiredParameters.add(Parameter((b) => b..name = 'req'..type = refer(endpoint.requestClassName)));
          }

          b.body = Block((b) {
            b.addExpression(refer('state').assign(refer('const AsyncValue.loading()')));
            
            final repoProvider = '${StringUtils.toLowerCamelCase(categoryName)}RepositoryProvider';
            final repoCall = refer('ref').property('read').call([refer(repoProvider)]).property(endpoint.methodName);
            ...
            b.addExpression(declareFinal('result').assign(repoCall.call(args).awaited));
            
            b.addExpression(refer('result').property('fold').call([
              Method((b) => b..requiredParameters.add(Parameter((b) => b..name = 'f'))..body = refer('state').assign(refer('AsyncValue.error').call([refer('f'), refer('StackTrace.current')])).statement).closure,
              Method((b) => b..requiredParameters.add(Parameter((b) => b..name = 's'))..body = refer('state').assign(refer('AsyncValue.data').call([refer('s')])).statement).closure,
            ]));
          });
        }));
    });
